...
const optionalDraftFields = [
  ...
  body('fatherName').optional({ values: 'falsy' }).isString().trim().isLength({ max: 100 }),
  ...
  body('clientSoftCode').optional().isString().trim().isLength({ max: 50 }),
  ...
  body('specialAllowance').optional().isFloat({ min: 0 }),
  body('grossSalary').optional().isFloat({ min: 0 }),
  body('isPfApplicable').optional().isBoolean().toBoolean(),
  body('isEsiApplicable').optional().isBoolean().toBoolean(),
  body('isLwfApplicable').optional().isBoolean().toBoolean(),
  body('employeePfPercentage').optional().isFloat({ min: 0, max: 100 }),
  body('employeeEsiPercentage').optional().isFloat({ min: 0, max: 100 }),
  body('employeeLwfPercentage').optional().isFloat({ min: 0, max: 100 }),
  body('employeePfMaxAmount').optional().isFloat({ min: 0 }),
  body('employeeEsiMaxAmount').optional().isFloat({ min: 0 }),
  body('employeeLwfMaxAmount').optional().isFloat({ min: 0 }),
...
const employeeBodyFields = [
  ...
  body('designationGradeId').optional().isString(),
  body('basicSalary').isFloat({ min: 0.01 }).withMessage('Basic salary must be greater than 0'),
  body('houseRentAllowance').optional().isFloat({ min: 0 }).withMessage('HRA must be >= 0'),
  body('specialAllowance').optional().isFloat({ min: 0 }).withMessage('Special allowance must be >= 0'),
  body('grossSalary').optional().isFloat({ min: 0 }),
  body('isPfApplicable').optional().isBoolean().toBoolean(),
  body('isEsiApplicable').optional().isBoolean().toBoolean(),
  body('isLwfApplicable').optional().isBoolean().toBoolean(),
...
export const updateEmployeeValidation = [
  ...
  body('employmentType').optional().isInt({ min: 1, max: 6 }).custom((v) => Object.values(EmploymentType).includes(v)),
